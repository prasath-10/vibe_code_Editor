...
if (terminalRef.current?.writeToTerminal) {
  terminalRef.current.writeToTerminal(
    "🚀 Detecting start script from package.json...\r\n"
  );
}
...
const preferredOrder = ["dev", "start", "serve"];

const scriptToRun = preferredOrder.find(
  (script) => packageJson.scripts?.[script]
);

if (!scriptToRun) {
  throw new Error(
    "❌ No runnable script found in package.json (dev/start/serve missing)"
  );
}

if (terminalRef.current?.writeToTerminal) {
  terminalRef.current.writeToTerminal(
    `▶ Running npm run ${scriptToRun}...\r\n`
  );
}
